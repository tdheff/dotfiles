...
return function()
        ...
        {
          {
            {
              id = "icon",
              image = gears.color.recolor_image(icondir .. "battery-unknown.svg", color["Text"]),
      ...
      function(stdout)
        ...
        if status == 'fully-charged' or status == 'charging' and battery_percentage == 100 then
          ...
          battery_widget.container.battery_layout.icon_margin.icon_layout.icon:set_image(gears.surface.load_uncached(
            gears.color.recolor_image(icondir .. icon .. '.svg', color["Text"])))
          return
        end
  ...
  Hover_signal(battery_widget, color["Love"], color["Text"])
  ...
  watch(
    ...
    function(widget, stdout)
      ...
      if status == nil or status == '' then
        ...
        battery_tooltip:set_text('No battery found')
        battery_widget.container.battery_layout.icon_margin.icon_layout.icon:set_image(gears.surface.load_uncached(
          gears.color.recolor_image(icondir .. 'battery-off' .. '.svg', color["Text"])))
      end
