...
return function()
        ...
        {
          {
            {
              id = 'icon',
              image = gears.color.recolor_image(icondir .. "no-internet" .. ".svg", color["Text"]),
    ...
    local update_wireless_icon = function(strength)
      awful.spawn.easy_async_with_shell(
        ...
        function(stdout)
          ...
          network_widget.container.network_layout.icon_margin.icon_layout.icon:set_image(gears.color.recolor_image(icondir .. icon .. ".svg", color["Text"]))
        end
      )
    end
  ...
  -- Signals
  Hover_signal(network_widget, color["Iris"], color["Text"])
