...
return function(s)
      ...
      awesome.connect_signal(
        ...
        function(new_node)
          if node == new_node then
            ...
            bg = color["Rose"]
            fg = color["Base"]
            device.background:set_bg(color["Rose"])
            device.background:set_fg(color["Base"])
          else
            fg = color["Rose"]
            bg = color["OverlayMed"]
            device.background:set_fg(color["Rose"])
            device.background:set_bg(color["OverlayMed"])
          end
        end
      )
      ...
      awesome.connect_signal(
        ...
        function(new_node)
          if node == new_node then
            ...
            bg = color["Dawn"]
            fg = color["Base"]
            device.background:set_bg(color["Dawn"])
            device.background:set_fg(color["Base"])
          else
            fg = color["Dawn"]
            bg = color["OverlayMed"]
            device.background:set_fg(color["Dawn"])
            device.background:set_bg(color["OverlayMed"])
          end
        end
      )
            ...
            {
              {
                {
                  resize = false,
                  image = gears.color.recolor_image(icondir .. "menu-down.svg", color["Rose"]),
                  widget = wibox.widget.imagebox,
                  id = "icon"
                },
                id = "center",
            ...
            {
              {
                {
                  resize = false,
                  image = gears.color.recolor_image(icondir .. "menu-down.svg", color["Pine"]),
                  widget = wibox.widget.imagebox,
                  id = "icon",
                },
                id = "center",
          ...
          {
            {
              resize = false,
              widget = wibox.widget.imagebox,
              image = gears.color.recolor_image(icondir .. "volume-high.svg", color["Rose"]),
              id = "icon",
            },
            {
              {
                ...
                bar_height = dpi(5),
                bar_color = color["Overlay"],
                bar_active_color = color["Rose"],
                handle_color = color["Rose"],
                handle_shape = gears.shape.circle,
                handle_border_color = color["Rose"],
                handle_width = dpi(15),
                maximum = 100,
          ...
          {
            {
              resize = false,
              widget = wibox.widget.imagebox,
              image = gears.color.recolor_image(icondir .. "microphone.svg", color["Dawn"]),
              id = "icon"
            },
            {
              {
                ...
                bar_height = dpi(5),
                bar_color = color["Overlay"],
                bar_active_color = color["Dawn"],
                handle_color = color["Dawn"],
                handle_shape = gears.shape.circle,
                handle_border_color = color["Dawn"],
                handle_width = dpi(15),
                maximum = 100,
                forced_height = dpi(26),
                widget = wibox.widget.slider,
  ...
  local function get_source_devices()
    awful.spawn.easy_async_with_shell(
      [[ pactl list sinks | grep -E 'Sink \#|device.description' | awk '{gsub(/"/, ""); for(i = 3;i < NF;i++) printf $i " "; print $NF}' ]],
  ...
  local function get_input_devices()
    awful.spawn.easy_async_with_shell(
      [[ pactl list sources | grep -E "node.name|device.description" | awk '{gsub(/"/, ""); for(i = 3;i < NF;i++) printf $i " "; print $NF}' ]],
  ...
  local function get_mic_volume()
    awful.spawn.easy_async_with_shell(
      ...
      function(stdout)
        ...
        if volume > 0 then
          volume_controller:get_children_by_id("mic_volume_margin")[1].icon:set_image(gears.color.recolor_image(icondir .. "microphone.svg", color["LightDawn"]))
        else
          volume_controller:get_children_by_id("mic_volume_margin")[1].icon:set_image(gears.color.recolor_image(icondir .. "microphone-off.svg", color["LightDawn"]))
        end
      end
    )
  end
  ...
  awesome.connect_signal(
    ...
    function(volume)
      ...
      volume_controller.controller_margin.controller_layout.audio_volume_margin.audio_volume.icon:set_image(gears.color.recolor_image(icon .. ".svg", color["Rose"]))
    end
  )
  ...
  awesome.connect_signal(
    ...
    function(mute)
      if mute then
        volume_controller.controller_margin.controller_layout.audio_volume_margin.audio_volume.icon:set_image(gears.color.recolor_image(icondir .. "volume-mute.svg", color["Rose"]))
      end
    end
  )
  ...
  awesome.connect_signal(
    ...
    function(volume)
      if volume > 0 then
        volume_controller:get_children_by_id("mic_volume_margin")[1].mic_volume.icon:set_image(gears.color.recolor_image(icondir .. "microphone.svg", color["LightDawn"]))
      else
        volume_controller:get_children_by_id("mic_volume_margin")[1].mic_volume.icon:set_image(gears.color.recolor_image(icondir .. "microphone-off.svg", color["LightDawn"]))
      end
    end
  )
